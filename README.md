...
blobs. Репозиторий также содержит воспроизводимый снимок локального
Speechmatics-пайплайна, который создаёт диаризованные расшифровки и
нормализованные таблицы. Секреты, медиа и рабочие результаты в Git не входят.
...
- [Как воспроизвести расшифровку и диаризацию](#speechmatics-пайплайн)
- [Полное руководство по Speechmatics-пайплайну](docs/speechmatics-pipeline.md)
...
flowchart LR
  ...
  subgraph OUT[Не входят в Release asset]
    ...
    CHATS[chats.db]
    CODE[код пайплайна<br/>лежит отдельно в Git]
  end
```
...
## Speechmatics-пайплайн

Полный очищенный исходный код расположен в
[`pipeline/speechmatics-speaker-archive/`](pipeline/speechmatics-speaker-archive/).
Это не содержимое release-asset `app.db`, а воспроизводимый генератор его
транскрипционного слоя.

Пошаговая эксплуатация, параметры, состояния очереди, регистрация профилей,
восстановление после ошибок и все форматы результата разобраны в
**[полном руководстве по Speechmatics-пайплайну](docs/speechmatics-pipeline.md)**.

```mermaid
flowchart TB
  A[Референсы спикеров] --> B[Ручное подтверждение]
  B --> C[Регистрация в Speechmatics]
  C --> D[Идентификаторы голосовых профилей]
  E[Архивные WAV<br/>read-only] --> F[Временный 16 kHz mono FLAC]
  D --> G[Speechmatics job]
  F --> G
  G --> H[Диаризация + слова + таймкоды]
  H --> I[(videos / video_speakers<br/>utterances / words)]
  I --> J[JSON / CSV / SRT / VTT / TXT]
  I --> K[Опциональное обогащение и поиск]
```

В Git включены FastAPI-панель, последовательный резюмируемый worker,
Speechmatics-клиент, подготовка FFmpeg, регистрация профилей, парсер JSON v2,
схема SQLite, миграции, экспорт, тесты и опциональный слой обогащения.
Исключены:

- `.env` и любые реальные API-ключи;
- исходные и подготовленные аудиофайлы;
- референсные WAV и `speaker_identifier` из рабочих данных;
- `app.db`, raw-транскрипты, клипы, кэши и логи заданий.
...
# Запишите новый APP_SECRET_KEY и свой SPEECHMATICS_API_KEY только в локальный .env
docker compose up --build -d
```

По умолчанию интерфейс доступен только через `127.0.0.1:8000`, а каталог WAV
монтируется в контейнер как `/audio:ro`. Подробности, порядок регистрации
профилей и схема потока находятся в
[`README пайплайна`](pipeline/speechmatics-speaker-archive/README.md), правила
защиты секретов — в
[`SECURITY.md`](pipeline/speechmatics-speaker-archive/SECURITY.md).
...
В этой публикации открыт Speechmatics-пайплайн и его общий слой обогащения.
Материализация именно аудированного release-снимка всё равно выполняется
сопровождающим: она зависит от непубликуемых медиа, ручных решений и приватной
рабочей среды. Ревьюер передаёт evidence через Issue, а опубликованный результат
можно независимо проверить командами из раздела «Проверка скачанного снимка».
